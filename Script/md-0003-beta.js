class CodeValidator {
    ...
    // 检查 PHP 语法（调用优化后的本地 API）
    async validatePHP(php) {
        if (!php.trim()) return;
        try {
            const response = await fetch('php/md-p0001.php', {
                method: 'POST',
            ...
            if (result.success) {
                ...
                // 如果有错误行号，则输出带行号的错误信息
                const errorMessage = result.line_number
                    ? `Error on line ${result.line_number}: ${result.error}`
                    : result.error;
                this.logError('PHP', errorMessage);
            }
        } catch (error) {
...
// 初始化代码验证器
const validator = new CodeValidator('Md-0003-beta');
